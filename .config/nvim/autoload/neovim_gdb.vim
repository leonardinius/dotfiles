...
function! s:Spawn(server_cmd, client_cmd, server_addr, reconnect)
  ...
  tnoremap <silent> <f5> <c-\><c-n>:GdbRun<cr>i
...
command! GdbDebugStart call s:Spawn(0, s:run_gdb, printf('localhost:%d', s:gdb_port), 0)
...
command! -nargs=1 -complete=file GdbInspectCore call s:Spawn(0, printf('gdb -q -f -c %s', <q-args>), 0, 0)
